...
const userSchema = new mongoose.Schema({
  ...
  avatar: {
    type: String,
    default: "https://res.cloudinary.com/dq7vq8fbj/image/upload/v1631017866/avatars/default_avatar_q6jv4b.png"
  },
  personalBests: {
    fiveK: {
      date: {
      ...
      time: {
        type: String,
        match: [/^(?:2[0-3]|[01][0-9]):[0-5][0-9]:[0-5][0-9]$/, "Please provide time in HH:MM:SS format"],
        default: null
      },
      timeInSeconds: {
        type: Number,
...
userSchema.pre("save", function(next) {
  ...
  races.forEach(race => {
    if (this.personalBests[race].time) {
      const [hours, minutes, seconds] = this.personalBests[race].time.split(":").map(Number);
      this.personalBests[race].timeInSeconds = (hours * 3600) + (minutes * 60) + seconds;
    }
  });
...
userSchema.methods.getFastestRace = function() {
  ...
  races.forEach(race => {
    if (this.personalBests[race].timeInSeconds) {
      let distanceInKm;
      switch(race) {
        case "fiveK": distanceInKm = 5; break;
        case "tenK": distanceInKm = 10; break;
        case "halfMarathon": distanceInKm = 21.0975; break;
